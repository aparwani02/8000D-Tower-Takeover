...
double stackUnloadPos = 1000.00; //was 1010
double moreThanSevenPos = 3000.0; //2428.8;
double firstStop = (5/8) * stackUnloadPos; //used when changing stack speed when going up
double secondStop = .75 * stackUnloadPos;
...
double midTowerPos = 620.20; //was 600.2
double lowTowerPos = 471.20; //was 451.2
double midTowerPosDropOff = 530.20; //was 540.2 560.20
double lowTowerPosDropOff = 381.20; //was 391.2 411.20;
double midTowerPosDeScore = 560.20;
double lowTowerPosDeScore = 411.20;
double grabberCurrentPos = 0.00;
bool grabberAtBottom = true;
bool grabberAtLow = false;
bool grabberAtMid = false;
bool loweredOnce = false;
bool lowTowerDeScore = false;
bool midTowerDeScore = false;
...
void flipOut(void) {
  ...
  chassis_move(-1.25, 25); //back up against the wall
...
void fiveCubesEXPERIMENTAL(void) {
  chassis_move(-1.25, 25); //back up against the wall
  chassisPIDMoveMax(48, 140); //was 120, was 40 //43
  l1PressedAuton();
  //vex::task::sleep(500);
  chassisPIDMoveMax(-31, 160); //was -23, was 140 //26
  
  vex::task::sleep(75);
  turn(-122, 60); //was 50; was -130, was -125, was -120
  vex::task::sleep(75); //was 150
  chassisPIDMoveMax(18, 130); //was 20
  upPressedAuton();
  
  while(stackInUnload == false);

  vex::task::sleep(250);
  downPressed();
  rightPressed();
  chassisPIDMoveMax(-6, 140);
  rightPressed();
  //}
}
...
void stackSpin(double velocity) {
  StackMotor.setStopping(vex::brakeType::hold);
  StackMotor.setVelocity(velocity,velocityUnits::rpm);
  
  StackMotor.spin(vex::directionType::fwd);
}
...
void setStackSmooth(int speed){
  double inertia = 0.97; // was 0.5
  static int currentSpeed = 0;
  currentSpeed = inertia * currentSpeed + (1 - inertia) * speed;
  stackSpin(currentSpeed);
}
...
void chassisPIDMoveMax(double inches, double maxSpeed){
   ...
   while(std::abs(errorL) > 20 || std::abs(errorR) > 20) {
     ...
     powerL = Limit((proportionalL + derivativeL) * 0.6, maxSpeed * -1 , maxSpeed); //* .6
     powerR = Limit((proportionalR + derivativeR) * 0.6, maxSpeed * -1 , maxSpeed); //* .6
...
void upPressedAuton() { //make stack vertical
  autonRecord.push_back( "upPressed();");
  stackCurrentPos = StackMotor.rotation(rotationUnits::deg);
  if(stackInStart == true) {
    StackMotor.setVelocity(100, velocityUnits::pct); //was 60 pct
    LeftGrabber.spin(directionType::fwd, 10, velocityUnits::pct); //was 3 pct
    RightGrabber.spin(directionType::fwd, 10, velocityUnits::pct); //was 3 pct
    StackMotor.rotateTo(stackUnloadPos/2, rotationUnits::deg); // was stackUnloadPos+35
    StackMotor.setVelocity(40, velocityUnits::pct);
    StackMotor.rotateTo(stackUnloadPos, rotationUnits::deg);
    StackMotor.stop(hold);
    LeftGrabber.stop(hold);
    RightGrabber.stop(hold);
...
void upPressedPID() { //make stack vertical

  double kPStack = 0.8; //was 1
  double kDStack = 1; //was 1 , 0.5
  ...
  double distance = StackMotor.rotation(rotationUnits::deg);

  double error = stackUnloadPos - distance;
  ...
  while(std::abs(error) > 20) {
    distance = StackMotor.rotation(rotationUnits::deg);

    error = stackUnloadPos - distance;

    proportional = error * kPStack;

    speed = lastError - error;

    derivative = -speed * kDStack;

    lastError = error;

    
    power = Limit((proportional + derivative) * 0.4,-160,160); //was .2 //.4 WORKING

    setStackSmooth(power);
...
void rightPressed() { //slow outtake
  ...
  if(RevGrabberStop) {
    LeftGrabber.spin(directionType::fwd, 15, velocityUnits::pct); //was 25 pct
    RightGrabber.spin(directionType::fwd, 15, velocityUnits::pct);  //was 25 pct
...
void xPressed() { //outtake macro
  ...
  rightPressed();
  autonRecord.push_back( "xPressed();");
  chassis_move_non_blocking(-12, 7);
  //chassisPIDMoveMax(-12, 10);
  downPressed();
}
void yPressed(){
  autonRecord.push_back( "yPressed();");
  StackMotor.setVelocity(200, velocityUnits::rpm);
  StackMotor.spin(directionType::rev);
  vex::task::sleep(500);
  StackMotor.stop(hold);
  StackMotor.resetRotation();
}
...
void aPressed() {
  ...
  if(grabberAtLow == true) {
    GrabberLift.setVelocity(-100, velocityUnits::pct);
    GrabberLift.rotateTo(lowTowerPosDeScore, rotationUnits::deg);
    GrabberLift.stop(hold);
    grabberCurrentPos = GrabberLift.rotation(rotationUnits::deg);
    grabberAtLow = false;
    grabberAtMid = true;
    grabberAtBottom = false;
    lowTowerDeScore = true;
    midTowerDeScore = false;
    loweredOnce = true;
    
  }

  else if(grabberAtMid == true) {
    GrabberLift.setVelocity(-100, velocityUnits::pct);
    GrabberLift.rotateTo(midTowerPosDeScore, rotationUnits::deg);
    GrabberLift.stop(hold);
    grabberCurrentPos = GrabberLift.rotation(rotationUnits::deg);
    grabberAtLow = false;
    grabberAtMid = true;
    grabberAtBottom = false;
    lowTowerDeScore = false;
    midTowerDeScore = true;
    loweredOnce = true;
...
void cubeOut() {
  ...
  if(grabberAtMid == true && loweredOnce == false) {
    GrabberLift.setVelocity(-100, velocityUnits::pct);
    GrabberLift.rotateTo(midTowerPosDropOff, rotationUnits::deg);
    GrabberLift.stop(hold);
    grabberCurrentPos = GrabberLift.rotation(rotationUnits::deg);
    grabberAtLow = false;
    grabberAtMid = true;
  ...
  else if(grabberAtLow == true && loweredOnce == false) {
    GrabberLift.setVelocity(-100, velocityUnits::pct);
    GrabberLift.rotateTo(lowTowerPosDropOff, rotationUnits::deg);
    GrabberLift.stop(hold);
    grabberCurrentPos = GrabberLift.rotation(rotationUnits::deg);
    grabberAtLow = true;
    grabberAtMid = false;
...
void usercontrol( void ) {
 ...
 //Callbacks
    ...
    Controller1.ButtonUp.pressed(*upPressedPID);
  ...
  while (1) {
    ...
    //Brain.Screen.print(GrabberLift.rotation(rotationUnits::deg));
